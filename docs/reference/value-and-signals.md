...
| `rosAdjust`   | `{ shelved, factor, projPosRank, expertPosRank }` — what the rest-of-season correction did (see [Rest-of-season realism](#rest-of-season-realism-added-in-v0150)). `rosPoints` / `rosValue` are already corrected.                 | player not in the build        |
